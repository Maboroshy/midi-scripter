...
def html_log_formatter(log_entries: list[LogEntry]) -> list[str]:
    ...
    for entry in log_entries:
        ...
        try:
            args = [_obj_ref_to_html(obj_ref) for obj_ref in entry.format_args]
            kwargs = {arg: _obj_ref_to_html(obj_ref) for arg, obj_ref in entry.format_kwargs.items()}
            text = text.format(*args, **kwargs)
        except (KeyError, IndexError):
            pass

        timestamp = _to_html_colored_text(f'{entry.timestamp}: ', 'grey')

        if entry.color:
            text = _to_html_colored_text(text, entry.color)

        text = text.replace('\n', '<br>')

        html_entry = f'<p>{timestamp}{text}</p>'
